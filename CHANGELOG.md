...
### Changed
- Docs: Update server.go documentation (PR: #1010)
- Chore: Fix godoc comment (PR: #1009)
...
- Introduced new configuration for `DelayedTaskCheckInterval`. See the [package documentation](https://pkg.go.dev/github.com/pars-aria-labs/asynq) for more details.
...
**IMPORTANT**: All `Inspector` related code moved to subpackage `github.com/pars-aria-labs/asynq/inspeq`.
...
- `Scheduler` type is added to enable periodic tasks. See the package documentation for its APIs.
...
**IMPORTANT**: If you are upgrading from a previous version, install the latest CLI with `go install github.com/pars-aria-labs/asynq/tools/asynq@latest` and run `asynq migrate`. No process should write to Redis while the migration runs.
...
- To upgrade the CLI, run `go install github.com/pars-aria-labs/asynq/tools/asynq@latest`.
