...
func NewClient(r RedisConnOpt) *Client {
	...
	client := newClientFromRedisClient(redisClient, redisPrefixFromConnOpt(r))
...
func NewClientFromRedisClient(c redis.UniversalClient) *Client {
	return newClientFromRedisClient(c, "")
}

func newClientFromRedisClient(c redis.UniversalClient, prefix string) *Client {
	return &Client{broker: rdb.NewRDB(c, prefix), sharedConnection: true}
}
